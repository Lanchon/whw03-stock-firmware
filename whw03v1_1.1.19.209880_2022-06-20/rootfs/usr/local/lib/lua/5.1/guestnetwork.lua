...
--
-- $Author: hsulliva $
-- $DateTime: 2021/08/27 10:12:29 $
-- $Id: //depot-irv/olympus/nodes_dev_tb/lego_overlay/proprietary/jnap/lualib/guestnetwork.lua#3 $
--
...
function _M.setIsGuestNetworkEnabled(sc, isGuestNetworkEnabled)
    sc:writelock()
    local ws
    if util.isModuleAvailable('wirelessscheduler') then
        ws = require('wirelessscheduler')
    end
    if not platform.isReady(sc) or (sc:get_wifi_status() ~= 'started' and not (ws and ws.getIsEnabled(sc) and ws.isCurrentTimeBlocked(sc))) then
