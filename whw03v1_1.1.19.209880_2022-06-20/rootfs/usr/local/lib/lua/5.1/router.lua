...
--
-- $Author: hsulliva $
-- $DateTime: 2022/01/07 12:25:24 $
-- $Id: //depot-irv/olympus/nodes_dev_tb/lego_overlay/proprietary/jnap/lualib/router.lua#9 $
--
...
_M.DEFAULT_LAN_IPADDRESS = '192.168.1.1'
...
function _M.setLANSettings(sc, settings)
    ...
    -- If the LAN IP address is being set to the default subnet (192.168.1.x/24)
    -- then disable the IPTV service
    if util.isModuleAvailable('iptv') and sc:get_iptv_enabled() then
        local ipAddress = tostring(settings.ipAddress)
        if settings.networkPrefixLength == 24 and ipAddress:match('^%d+.%d+.%d') == _M.DEFAULT_LAN_IPADDRESS:match('^%d+.%d+.%d') then
            sc:set_iptv_enabled(false)
        end
    end
