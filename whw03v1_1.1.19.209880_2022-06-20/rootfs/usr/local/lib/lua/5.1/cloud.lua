...
function _M.getAssetData(sc, endPointName)
    -- Ensure no lock is held when making this call as it is blocking
    assert(not sc:isreadlocked() and not sc:iswritelocked(), 'must not hold the sysctx lock when calling getAssetData()')
    ...
    ownedsc:rollback()
    local response, status, message = cloud.callCloud({
        host = cloudHost,
        path = '/asset-service/rest/assets/'..endPointName,
        method = 'GET',
        headers = {
            ['X-Linksys-Client-Type-Id'] = cloud.ROUTER_CLIENT_TYPE_ID,
            ['X-Linksys-Network-Id'] = networkId,
            ['X-Linksys-Network-Secret'] = networkPassword
        },
        verifyPath = verifyHost and CA_PATH or nil
    })
