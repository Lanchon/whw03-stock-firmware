...
function _M.addLanToLanPinhole(sc, pinhole)
    sc:writelock()

    local id = platform.getUUID()
    sc:add_lantolan_firewall_pinhole(id, tostring(pinhole.ipAddress), pinhole.port, pinhole.protocol)

    return nil, hdk.uuid(id)
end

function _M.deleteLanToLanPinhole(sc, pinholeID)
    sc:writelock()
    if not sc:delete_lantolan_firewall_pinhole(tostring(pinholeID)) then
        return 'ErrorPinholeNotFound'
    end
end

function _M.getLanToLanPinholes(sc)
    sc:readlock()

    local pinholeData = sc:get_lantolan_firewall_pinholes()
    local pinholes = {}

    for i = 1, #pinholeData do
        table.insert(pinholes, {
            pinholeID = hdk.uuid(pinholeData[i].id),
            pinholeInfo = {
                ipAddress = hdk.ipaddress(pinholeData[i].ipAddress),
                port = pinholeData[i].port,
                protocol = pinholeData[i].protocol
            }
        })
    end

    return nil, { pinholes = pinholes }
end
