...
ulog system status "$PID system is thinking about committing syscfg"
   if [ "$no_syscfg_commit" != "1" ] ; then
      ...
      syscfg lockdown
      sync
      if [ "`syscfg get system_backup_on_reboot`" == "1" ] ; then
				sysevent set sys_recovery::backup
      fi
