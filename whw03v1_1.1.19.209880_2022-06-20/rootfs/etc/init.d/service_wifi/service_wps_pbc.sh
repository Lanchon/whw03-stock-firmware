...
case "$1" in
	...
	${SERVICE_NAME}-restart)
		...
		;;
	${EVENT_HW_BUTTON})
		if [ "`cat /etc/product`" != "nodes" ] && [ "`cat /etc/product`" != "dallas" ] && [ "`cat /etc/product`" != "rogue" ] && [ "`cat /etc/product`" != "lion" ] ; then
			if [ "disabled" = "`syscfg get wl0_wps_state`" ] && [ "disabled" = "`syscfg get wl1_wps_state`" ]; then
				exit
			fi
		else
			if [ "disabled" = "`syscfg get wl0_wps_state`" ] && [ "disabled" = "`syscfg get wl1_wps_state`" ] && [ "disabled" = "`syscfg get wl2_wps_state`" ]; then
				exit
			fi
		fi
