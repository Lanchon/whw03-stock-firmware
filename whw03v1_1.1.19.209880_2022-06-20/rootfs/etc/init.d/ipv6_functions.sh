...
{
   LAN_ROUTE=`ip -6 route show | grep 'unreachable\|blackhole' | awk '{ print $2 }'`
   for loop in $LAN_ROUTE
   do
       LINE=`ip -6 route show | grep 'unreachable\|blackhole' | grep $loop`
       LINE=`echo $LINE | awk '{ print $2 " " $3 " " $4 }'`
       if [ -n "$LINE" ] ; then
          echo "delete unreachable and blackhole route $LINE" > /dev/console
          ip -6 route del $LINE
       fi
   done
}
...
async_disc_ip6_to_ddb()
{
   (
   set -e
   eval `utctx_cmd get lan_ifname`
   flock -n 100
   ping6 -q -I $SYSCFG_lan_ifname -c 1 ff02::1 && sleep 11
   ip -6 neigh show dev $SYSCFG_lan_ifname nud reachable | while read LINE; do
      IP6=$(echo "$LINE" | cut -d ' ' -f 1)
      MAC=$(echo "$LINE" | cut -d ' ' -f 3)
      devicedb_client -c addIp "$MAC" "$IP6" ipv6
   done
   ) 100> /var/lock/ndp_$SYSCFG_lan_ifname.lock &
}
