...
service_start () 
{
   ...
   if [ ! -e $CRONTAB_FILE ] ; then
      ...
      if [ "`cat /etc/product`" = "nodes" ] || [ "`cat /etc/product`" = "wraith" ] || [ "`cat /etc/product`" = "nodes-jr" ] || [ "`cat /etc/product`" = "rogue" ] || [ "`cat /etc/product`" = "lion" ] ; then
      	echo "* * * * * /etc/init.d/service_wifi/smart_connect_client_connection_monitor.sh" >> $CRONTAB_FILE
      	echo "* * * * * /etc/init.d/service_wifi/smart_connect_VAP_monitor.sh" >> $CRONTAB_FILE
        if [ "$(syscfg get smart_mode::mode)" != "1" ] ; then
            echo "* * * * * /etc/init.d/service_lldpd.sh lldp::update" >> $CRONTAB_FILE
        fi
        if [ "$(syscfg get smart_mode::mode)" != "2" ] ; then
            echo "* * * * * /etc/init.d/service_backhaul_switching/backhaul_check.sh" >> $CRONTAB_FILE
        fi
        echo "* * * * * /bin/cleanup_msearch_allowance_rules.sh" >> $CRONTAB_FILE
      fi
      ...
      echo "36 3 * * * /etc/devicedb/mem_workaround.sh" >> $CRONTAB_FILE
      
      USER_SCHED_TAB="`syscfg get user_sched_tab`"
      if [ "$USER_SCHED_TAB" ] && [ -f "$USER_SCHED_TAB" ] ; then
				echo "adding users scheduled tasks to cron..."
				cat $USER_SCHED_TAB >> $CRONTAB_FILE
      else
				echo "no user scheduled tasks found..."
      fi
      ln -sf /usr/bin/cronExpireSessions /etc/cron/cron.everyminute
   fi
   
   if [ -f "/etc/sys_backup.sh" ] && [ ! -f "/tmp/cron/cron.daily/sys_backup.sh" ] ; then
		cp /etc/sys_backup.sh /tmp/cron/cron.daily/
	 fi
