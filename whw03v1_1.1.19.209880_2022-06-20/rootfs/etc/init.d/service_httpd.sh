...
do_start_lighttpd() {
   ...
   async_id=`sysevent async firewall-status $SELF_BIN/firewall-change-handler`
   sysevent set httpd_firewall_asyncid "$async_id"
   logger -t ESM $(printf "Registering for firewall-status changes returns %s $async_id")
   $BIN -f $CONF_FILE
...
service_start ()
{
    ...
    if is_lighttpd_running ; then
            ...
            JNAP_STATUS=`sysevent get jnap-status`
            if [ "ready" != "$JNAP_STATUS"  ] ; then
                echo "Not ready for JNAP. Sleeping for 15 seconds..." > /dev/console
                sleep 15
                sysevent set jnap-status "ready"
            fi
    else
...
case "$1" in
  ...
  omsg::location)
      ...
      ;;
  *)
        echo "Usage: $SELF_NAME [${SERVICE_NAME}-start|${SERVICE_NAME}-stop|${SERVICE_NAME}-status|${SERVICE_NAME}-restart|lan-started|lan-stopping|omsg::location]" >&2
        exit 3
        ;;
esac
