...
cloud_upload_files()
{
   ...
   if [ $# -lt 3 ]; then
      ...
      CLOUD_ERROR_CODE=INVALID_PARAMETER
      CLOUD_ERROR_DESC="Invalid upload type (CRASH_LOG|SYSINFO_LOG)"
      rc=3
...
EOF
      ...
      if [ $rc -ne 0 ]; then
         ...
         if [ $found -gt 0 ]; then
            ...
            if [ "$found" ]; then
               ...
               sysevent set sysinfo::upload_id $uploadId
            else  # unknown error
               rc=3
            fi
         fi
   ...
   if [ $rc -ne 0 ]; then
      ...
      sysevent set sysinfo::upload_error_code $CLOUD_ERROR_CODE
      sysevent set sysinfo::upload_error_desc $CLOUD_ERROR_DESC
   fi
   return $rc
}
configure_cron_jobs()
...
upload_crash_log()
{
   ...
   if (echo $dataTypes | grep sysinfo > /dev/null); then
      /www/sysinfo_json.cgi CMD=sync > $SYSINFO_FILE 2>/dev/null
...
upload_sysinfo()
{
   ...
   /www/sysinfo_json.cgi CMD=sync SECTION="$sections" > $SYSINFO_FILE 2>/dev/null
...
case "$1" in
   ...
   user_consented_data_upload)
      ...
      ;;
   *)
      echo "Usage: $SELF_NAME [${SERVICE_NAME}-start|${SERVICE_NAME}-stop|${SERVICE_NAME}-restart|wan-started|wan-stopped|device_registered|upload_crash_log|upload_sysinfo <reason> [<sections>]" >&2
      exit 3
      ;;
esac
