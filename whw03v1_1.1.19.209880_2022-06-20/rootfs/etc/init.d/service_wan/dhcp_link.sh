...
do_start_dhcp() {
   ...
   if [ "pppoe" != "$SYSCFG_wan_proto" ] ; then
      ...
      if [ "1" = "$SYSCFG_dhcp_option_6rd_enable" ] ; then
        ...
        if [ -n "$SYSCFG_clientid" -a "$SYSCFG_clientid" != "" ] ; then
            option_clientid="-c $SYSCFG_clientid"
        else
            option_clientid=""
        fi
        if [ -n "$SYSCFG_vendorclass" -a "$SYSCFG_vendorclass" != "" ] ; then
            opt_125=$(udhcpc_option125_string)
            option_vendorclass="-V $SYSCFG_vendorclass -x $opt_125"
        else
            option_vendorclass=""
        fi
      ...
      if [ -z "$UDHCPC_PID" ] ; then
         ...
         udhcpc -R -S -b $option_clientid $option_vendorclass -i $SYSCFG_ifname $HOSTNAME_OPT -p $UDHCPC_PID_FILE --arping -s $UDHCPC_SCRIPT $UDHCPC_OPTIONS $UDHCPC_EXTRA_PARAMS &
