...
if [ "stopped" = "$WAN_STATUS" ] ; then
   if [ "stopped" != "`sysevent get wan-status`" ] ; then
      ...
      if [ -z "$SYSEVENT_current_wan" ]; then
          SYSEVENT_current_wan="$DEFAULT_WAN"
      fi
      ...
      DEF_ROUTE=`ip -4 route show dev $SYSEVENT_current_wan_ifname | grep default`
      if [ -n "$DEF_ROUTE" ]; then
      ...
      if [ "$(syscfg get smart_mode::mode)" = "1" ] && [ "$(syscfg get bridge_mode)" = "1" ]; then
        ulog wan_status_monitor status "Now it is slave mode, do not flush the default router here.."
      else
   ...
   if [ "$wan_intf_auto_detect_enabled" = "1" ]; then
       if [ -z "$wan_detected_type" -o "$wan_detected_type" = "false" ]; then
           syscfg set wan::detected_type $SYSCFG_wan_proto 
       fi
       syscfg set wan::intf_auto_detect_enabled 0
       syscfg commit
       sysevent set wan_intf_auto_detect-restart
   fi
