...
bring_wan_up() {
   ...
   if [ "pptp" = "$SYSCFG_wan_proto" ] ; then
      ...
      if [ "$SYSCFG_pptp_address_static" == "0" ]; then
          outer_ipaddr=`sysevent get ${NAMESPACE}_ipv4_wan_ipaddr`
          outer_subnet=`sysevent get ${NAMESPACE}_ipv4_wan_subnet`
      else
          outer_ipaddr=`syscfg get wan_ipaddr`
          outer_subnet=`syscfg get wan_sunbet`
      fi
      if [ -n "$outer_ipaddr" -a "$outer_ipaddr" != "0.0.0.0" ]; then
          if [ -z "$outer_subnet" ]; then
              outer_subnet="255.255.255.0"
          fi
          calculate_lan_networks "$outer_ipaddr" "$outer_subnet"
      fi
      sysevent set pppd_current_wan_subnet
      sysevent set pppd_current_wan_ifname
      ...
      prepare_pptp
      sysevent set ${NAMESPACE}-errinfo
      sysevent set ppp_last_conn_err
