...
ipv6_wan_down() {
   echo "ipv6_wan_down: entry; IPV6_WAN_PROTO=$IPV6_WAN_PROTO" >> $LOG
...
service_init ()
{
   ...
   if [ "1" = "$SYSCFG_ipv6_static_enable" ] ; then
      ...
      IPV6_WAN_PROTO=bridge
      syscfg set dhcpv6c_enable 0
      syscfg set dhcpv6s_enable 0
      syscfg set router_adv_enable 0 
...
if [ "0" = "`syscfg get ipv6_enable`" ] ; then
   echo "ipv6_enable is 0; calling service_stop to disable ipv6" >> $LOG
   service_stop
   exit 0
else
