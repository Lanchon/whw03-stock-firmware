...
-- updatefirmware, and restoreconfiguration, and backupconfiguration
--
...
"result": "%s"
    ...
    if content then
        io.write('Content-Disposition: attachment; filename=backup.cfg\r\n')
    end
...
local function handleMultipartPOSTRequest(sc, boundary)
    ...
    while true do
        ...
        if handler == actionHandlers['backupconfiguration'] and (authorization or session) then
            break -- we have all we need for backupconfiguration
        end
    end
