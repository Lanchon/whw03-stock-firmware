...
DEVICEDB_BIN=/usr/sbin/devicedb
service_start ()
{
    eval `utctx_cmd get bridge_mode`
    if [ -f $DEVICEDB_BIN ] ; then
        DEVICEDB_STATUS=`sysevent get devicedb-status`
        LAN_STATUS=`sysevent get lan-status`
        if [ "started" != "$DEVICEDB_STATUS" ] || [ "started" != "$LAN_STATUS" ] || [ "$SYSCFG_bridge_mode" != "0" ] ; then
            ulog ${SERVICE_NAME} status "not running ${SERVICE_NAME} service because devicedb-status is ${DEVICEDB_STATUS}, lan-status is ${LAN_STATUS}, and bridge_mode is ${SYSCFG_bridge_mode}."
            return 0
        fi
    else
