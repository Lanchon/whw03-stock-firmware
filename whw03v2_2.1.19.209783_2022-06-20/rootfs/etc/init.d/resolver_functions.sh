...
reset_ethernet_ports() {
    if [ "`syscfg get hardware_vendor_name`" = "Broadcom" ] ; then
        ...
        if [ "`cat /etc/product`" = "wraith" ] ; then
            ...
            lan_ports="1 2 3 4"
        elif [ "`cat /etc/product`" = "nodes" -o "`cat /etc/product`" = "nodes-jr" ] ; then
            wan_intf="`sysevent get wan::detected_intf`"
            echo "wan_intf is $wan_intf" > /dev/console
            if [ -n "$wan_intf" ]; then
                if [ "`syscfg get switch::router_1::ifname`" = "$wan_intf" ]; then
                    lan_port_num=`syscfg get switch::router_2::port_numbers`
                else
                    lan_port_num=`syscfg get switch::router_1::port_numbers`
                fi
            
                if [ "$lan_port_num" = "4" ]; then
                    lan_ports="3"
                elif [ "$lan_port_num" = "5" ]; then
                    lan_ports="4"
                else
                    echo "lan port num is not correct, please check" > /dev/console
                    return
                fi
                wait_time="2.5"
                echo "power cycle port $lan_port_num" > /dev/console
            else
                return
            fi
        elif [ "`cat /etc/product`" = "civic" -o "`cat /etc/product`" = "dallas" -o "`cat /etc/product`" = "rogue" -o "`cat /etc/product`" = "lion" ] ; then
            lan_ports="0 1 2 3"
        fi
        if [ "`cat /etc/product`" = "nodes" -a "`cat /tmp/nodes_hw_version`" = "2" ] ; then
            lan_ports=${lan_ports/3/27}
            lan_ports=${lan_ports/4/28}
        fi
        for i in $lan_ports; do
...
reset_ethernet_port_with_portnum() {
    wait_time="2.5"
    lan_ports="$1"
    if [ "`cat /etc/product`" = "nodes" -a "`cat /tmp/nodes_hw_version`" = "2" ] ; then
        lan_ports=${lan_ports/3/27}
