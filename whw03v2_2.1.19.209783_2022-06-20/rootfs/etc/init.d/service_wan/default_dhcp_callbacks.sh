...
case "$1" in
   ...
   deconfig)
      ...
      ulog default_dhcp_cb status "$PID deconfig clear old ip assigned"
   ...
   renew|bound)
      ...
      echo "ntp server    : $ntpsvr"    >> $DEFAULT_LOG_FILE
      ...
      echo "vendorinfo    : $vendorinfo" >> $DEFAULT_LOG_FILE
      echo "vendorspecific: $vendorspecific" >> $DEFAULT_LOG_FILE
      LOG_FILE="/tmp/"${NAMESPACE}"_udhcpc.log"
      ...
      if [ -n "$vendorinfo" ] ; then
		...
		if [ -n "$ACSURL" ] ; then
			...
			if [ "$SYSCFG_acs_discovery_url" != "$ACSURL" ] ; then
				...
				ulog default_dhcp_cb info "++++ set ccsp-restart by new url: $ACSURL"
			fi
		fi
      fi
      if [ -n "vendorspecific" ] ; then
		...
		if [ -n "$GTW_OUI" ] ; then
			if [ "$SYSEVENT_gateway_OUI" != "$GTW_OUI" ] ; then
				...
				ulog default_dhcp_cb info "set GatewayManufacturerOUI: $GTW_OUI"
			fi
		else
			if [ -n "$SYSEVENT_gateway_OUI" ] ; then
				sysevent set gateway_OUI
				ulog default_dhcp_cb info "unset GatewayManufacturerOUI"
			fi
		fi
		...
		if [ -n "$GTW_SN" ] ; then
			if [ "$SYSEVENT_gateway_SN" != "$GTW_SN" ] ; then
				...
				ulog default_dhcp_cb info "set GatewaySerialNumber: $GTW_SN"
			fi
		else
			if [ -n "$SYSEVENT_gateway_SN" ] ; then
				sysevent set gateway_SN
				ulog default_dhcp_cb info "unset GatewaySerialNumber"
			fi
		fi
		...
		if [ -n "$GTW_CLASS" ] ; then
			if [ "$SYSEVENT_gateway_CLASS" != "$GTW_CLASS" ] ; then
				...
				ulog default_dhcp_cb info "set GatewayProductClass: $GTW_CLASS"
			fi
		else
			if [ -n "$SYSEVENT_gateway_CLASS" ] ; then
				sysevent set gateway_CLASS
				ulog default_dhcp_cb info "unset GatewayProductClass"
			fi
		fi
      fi
      if [ -n "$serverid" ] ; then
