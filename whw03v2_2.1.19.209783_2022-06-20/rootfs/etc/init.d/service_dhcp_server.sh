...
if [ "`syscfg get lrhk::enabled`" == "1" ] && [ -e /tmp/cedar_support ] ; then
    DB_LOCK="`syscfg get lrhk::shm_db_lock`"
    WL_FLAG="--lrhk $DB_LOCK"
else
    WL_FLAG=""
fi
...
service_init ()
{
    FOO=`utctx_cmd get lan_ipaddr lan_netmask bridge_ipaddr bridge_netmask dhcp_server_enabled bridge_mode dhcp_nameserver_1 dhcp_nameserver_2 dhcp_nameserver_3 smart_mode::mode`
    eval $FOO
    bridge_master=0
...
restart_request ()
{
   ...
   rm -f $DHCP_TMP_CONF
   CHECK=`ps | grep dnsmasq | grep -v grep |cut -d "/" -f 3`
   if echo "$CHECK" | grep -q "lrhk"; then
      if [ -z "$WL_FLAG" ]; then
         RESTART=1
	  fi
   else
...
dhcp_server_stop ()
{
   ...
   $SERVER $WL_FLAG $DEBUG_FLAG -u root -P 4096 -C $DHCP_CONF -8 $LOG
...
services_start()
{
    if [ "$SYSCFG_bridge_mode" != "0" ] ; then
      if [ "$SYSCFG_smart_mode_mode" = "2" ]; then
        ulog dhcp_server status "will run dhcp server on master mode bridge."
        bridge_master=1
      else
        return 0
      fi
   ...
   then
      ...
      if [ "$LOG_LEVEL" = "0" ] ; then
        ...
        $SERVER $WL_FLAG $DEBUG_FLAG -u root --dhcp-authoritative --disable_dhcp_log $DISABLE_DHCP_LOG -P 4096 -C $DHCP_CONF -8 $LOG $ORDER
   ...
   if [ "$SYSCFG_smart_mode_mode" = "2" ] && [ "`sysevent get smart_connect::setup_conflict_resolved`" = "1" ] ; then
       sysevent set smart_connect::setup_conflict_resolved
       sysevent set smart_connect::setup_ip_changed
   fi
}
services_stop()
{
