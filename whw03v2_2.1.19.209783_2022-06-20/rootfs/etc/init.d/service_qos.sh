...
service_start_on_wan ()
{
   ...
   do
      ...
      if [ "started" = "$LOCAL_STATUS" -a -n "$SYSEVENT_current_wan_ifname" ] ; then
         ...
         RES=`echo $SYSCFG_wan_upload_speed | awk '{if ($1 >= 314) print 1; else print 0}'`
         if [ "$SYSCFG_wan_upload_speed" = "0" ] || [ "$SYSCFG_wan_upload_speed_unit" = "2" -a "$RES" = "1" ]; then
             return
         else
...
service_start ()
{
   if [ "`syscfg get smart_mode::mode`" == "1" ] ; then
      ulog ${SERVICE_NAME} status "Qos is not supported on SLAVE."
      return
   fi
...
is_speed_test_running ()
{
     if [ "$(sysevent get speedtest::running)" == "1" ] ; then
        ulog ${SERVICE_NAME} status "speedtest::running So ignore the request, will do qos start later."
        echo 1
     elif [ "$(sysevent get speedtest::running)" == "0" ] && [ "$(sysevent get healthcheck::speedtest)" != "done" ] ; then
        ulog ${SERVICE_NAME} status "healthcheck::speedtest no finished, So ignore the request, will do qos start later."
        echo 1
     else
        echo 0
     fi  
...
case "$1" in
  ${SERVICE_NAME}-start)
     ...
     if [ "`syscfg get bridge_mode`" = "0" ] && [ "`sysevent get lan-status`" != "started" ]; then
        ulog ${SERVICE_NAME} status "LAN is not started. So ignore the request"
        exit 0
     fi
     if [ "$(is_speed_test_running)" = "1" ] ; then
        sysevent set qos_start_later 1
        exit 0
     fi     
     service_start
     if [ -n "$SYSCFG_qos_enable" -a "0" = "$SYSCFG_qos_enable" ] ; then
  ...
  ${SERVICE_NAME}-restart)
     if [ "`syscfg get smart_mode::mode`" == "1" ] ; then
        if [ "`sysevent get qos-status`" == "started" ] ; then
            service_stop
            sysevent set qos_enabled 0
            stop_wan_bandwidth_monitor
        fi
  ...
  recalculate_wan_shaping)
     if [ "`syscfg get smart_mode::mode`" == "1" ] ; then
     　　　ulog ${SERVICE_NAME} status "Ignore recalculate_wan_shaping on SLAVE."
    　　　 exit 0
     fi  
     for loop in br0
      do
  ...
  current_wan_ifname)
     ...
     if [ "started" = "$SERVICE_STATUS" ] && [ "`syscfg get smart_mode::mode`" != "1" ] ; then
        service_start_on_wan
            if [ -n "$SYSCFG_qos_enable" -a "0" = "$SYSCFG_qos_enable" ] ; then
               sysevent set qos_enabled 0
               stop_wan_bandwidth_monitor
     ...
     ;;
  wifi_guest-status)
    if [ "`sysevent get wifi_guest-status`" = "started" ] && [ "`syscfg get smart_mode::mode`" != "1" ] ; then
	service_start
    fi
    ;;  
  wifi-status)
    if [ "`sysevent get wifi-status`" = "started" ] && [ "`syscfg get smart_mode::mode`" != "1" ] ; then
	service_start
    fi
    ;;
  healthcheck::speedtest)
    if [ "$2" = "done" ] && [ "$(sysevent get qos_start_later)" = "1" ]; then
      sysevent set qos_start_later
      if [ "1" = "$SYSCFG_qos_enable" ] ; then
        ulog qos status "As Qos start hanged by HealthCheck, speedtest::running completed, so do restart/start qos"
        if [ -n "$(sysevent get qos-status)" ] && [ "$(sysevent get qos-status)" = "started" ] ; then
          /etc/init.d/service_qos.sh qos-restart
        else
          /etc/init.d/service_qos.sh qos-start
        fi     
      fi  
    fi
    ;;      
