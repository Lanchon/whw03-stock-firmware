...
service_start ()
{
   ...
   if [ "$WAN_INTF_AUTO_DETECT_ENABLED" = "1" ]; then
       ...
       if [ -z "$WAN_DETECTED_INTF" ]; then
           ulog wan status "wan port dynamic detection mode, but wan port does not exist"
           exit 1
       else
           ulog wan status "wan port dynamic detection mode, wan interface exists, going on"
       fi
   else
       ulog wan status "wan port fixed mode"
   fi
