...
register_monitor (){
   ...
   if [ "\$IGD_PID" = "" ] ; then
     ulog igd error "igd aborted, restart"  
     sysevent set igd-restart
   elif [ \$CPU_USED -gt $WARN_AT ] ; then
...
service_stop () {
   ...
   if [ "stopped" = "`sysevent get igd-status`" ] ; then
      ulog ${SERVICE_NAME} status "${SERVICE_NAME} service already stopped"
      exit
   fi
   stopigd
   sysevent set ${SERVICE_NAME}-errinfo
