...
bring_wan_up() {
   ...
   if [ "1" -eq "$SYSCFG_router_adv_provisioning_enable" ] ; then
      if [ -d /proc/sys/net/ipv6/conf/${SYSEVENT_current_wan_ipv6_ifname} ] ; then
         ...
         case "$SYSCFG_wan_if_v6_forwarding" in
		 0|1) # our WAN side should be like a router or host, 1 for router, 0 for host
			 echo "$SYSCFG_wan_if_v6_forwarding" > /proc/sys/net/ipv6/conf/${SYSEVENT_current_wan_ipv6_ifname}/forwarding
			 ;;
		*) # set WAN interface act as router by default
			 echo 1 > /proc/sys/net/ipv6/conf/${SYSEVENT_current_wan_ipv6_ifname}/forwarding
			 ;;
	 esac
