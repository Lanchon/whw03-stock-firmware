...
prepare_well_known_dns () {
   FOO=`utctx_cmd get lan_ifname dot_local_domain dot_local_hostname ula_enable lan_ula_prefix ui::remote_host cloud::host routerstatus::host xmpp_host hist_upload::host device::modelNumber private_domain`
   eval $FOO
   ...
   if [ -n "$SYSCFG_hist_upload_host" ]
   then
      echo "server=/${SYSCFG_hist_upload_host}/#" >> $1
   fi
...
prepare_smartconn_dhcp_conf () {
   SC_DHCP_LEASE_TIME="3600"
   SC_LAN_IPADDR="172.19.113.1"
   SYSCFG_smartconn_br_intf="`syscfg get smart_connect::br_dev`"
   echo "interface=$SYSCFG_smartconn_br_intf" >> $LOCAL_DHCP_CONF
   echo "dhcp-range=net:$SYSCFG_smartconn_br_intf,172.19.113.10,172.19.113.100,255.255.0.0,3600" >> $LOCAL_DHCP_CONF
}
calculate_ldal_wl_setup_vap_dhcp_range () {
    LDAL_WL_SETUP_VAP_DHCP_START=`syscfg get ldal_wl_setup_vap_dhcp_start`
    LDAL_WL_SETUP_VAP_DHCP_NUM=`syscfg get ldal_wl_setup_vap_dhcp_num`
    LDAL_WL_SETUP_VAP_SUBNET=`syscfg get ldal_wl_setup_vap_subnet`
    if [ "$LDAL_WL_SETUP_VAP_DHCP_START" -lt "2" ] ; then
        LDAL_WL_SETUP_VAP_DHCP_START=2
    fi
    LDAL_WL_SETUP_VAP_DHCP_END=`expr $LDAL_WL_SETUP_VAP_DHCP_START + $LDAL_WL_SETUP_VAP_DHCP_NUM`
    LDAL_WL_SETUP_VAP_DHCP_END=`expr $LDAL_WL_SETUP_VAP_DHCP_END - 1`
    LDAL_WL_SETUP_VAP_NETWORK_ADDR=`echo $LDAL_WL_SETUP_VAP_SUBNET | cut -f 1,2,3 -d '.'`
    LDAL_WL_SETUP_VAP_DHCP_START_ADDR=$LDAL_WL_SETUP_VAP_NETWORK_ADDR.$LDAL_WL_SETUP_VAP_DHCP_START
    LDAL_WL_SETUP_VAP_DHCP_END_ADDR=$LDAL_WL_SETUP_VAP_NETWORK_ADDR.$LDAL_WL_SETUP_VAP_DHCP_END
}
prepare_ldal_wl_setup_vap_dhcp_conf () {
    LDAL_WL_SETUP_VAP_DHCP_LEASE_TIME=`syscfg get ldal_wl_setup_vap_dhcp_lease_time`
    SVAP_LAN_IFNAME=`syscfg get svap_lan_ifname`
    LDAL_WL_SETUP_VAP_NETMASK=`syscfg get ldal_wl_setup_vap_netmask`
    LDAL_WL_SETUP_VAP_IPADDR=`syscfg get ldal_wl_setup_vap_ipaddr`
   if [ -z "$LDAL_WL_SETUP_VAP_DHCP_LEASE_TIME" ]; then
      LDAL_WL_SETUP_VAP_DHCP_LEASE_TIME=3600
   fi
   if [ -n "$DHCP_LEASE_TIME" ] && [ $DHCP_LEASE_TIME -lt $LDAL_WL_SETUP_VAP_DHCP_LEASE_TIME ]; then
      LDAL_WL_SETUP_VAP_DHCP_LEASE_TIME=$DHCP_LEASE_TIME
   fi
    calculate_ldal_wl_setup_vap_dhcp_range
    echo "interface=$SVAP_LAN_IFNAME" >> $1
    echo "dhcp-range=net:$SVAP_LAN_IFNAME,$LDAL_WL_SETUP_VAP_DHCP_START_ADDR,$LDAL_WL_SETUP_VAP_DHCP_END_ADDR,$LDAL_WL_SETUP_VAP_NETMASK,$LDAL_WL_SETUP_VAP_DHCP_LEASE_TIME" >> $1
    echo "tag:$SVAP_LAN_IFNAME,option:router,$LDAL_WL_SETUP_VAP_IPADDR" >> $2
    echo "tag:$SVAP_LAN_IFNAME,option:dns-server,$LDAL_WL_SETUP_VAP_IPADDR" >> $2
}
prepare_dhcp_conf_bridge () {
   ...
   prepare_guest_dhcp_conf
   prepare_guest_dhcp_options
   prepare_ldal_wl_setup_vap_dhcp_conf $LOCAL_DHCP_CONF $LOCAL_DHCP_OPTIONS_FILE
...
prepare_dhcp_conf () {
   ...
   if [ "" = "$Product" ] ; then
      Product=`syscfg get device::model_base`
   fi
   echo "$PREFIX""dhcp-option-force=cpewan-id,vi-encap:3561,6,\"$Product\"" >> $LOCAL_DHCP_CONF
   if [ "$LOG_LEVEL" -gt 1 ] ; then
      echo "$PREFIX""log-dhcp" >> $LOCAL_DHCP_CONF
   fi
   echo "interface=ath4" >> $LOCAL_DHCP_CONF
   echo "dhcp-range=net:ath4,172.31.255.100,172.31.255.159,255.255.255.0,3600" >> $LOCAL_DHCP_CONF
   if [ "dns_only" != "$3" ] ; then
      prepare_dhcp_conf_static_hosts
      prepare_dhcp_options
      prepare_guest_dhcp_conf
