...
local smartMode = sc:getinteger('smart_mode::mode', 0) -- 0 is unconfigured, 1 is slave, 2 is master

local masterIpAddress = nil
if smartMode == 1 then
    local location = sc:getevent('omsg::location')
    masterIpAddress = string.match(location, "(%d+.%d+.%d+.%d+)")
end
...
local function WriteCGIPagesRules(allowAccess)
    ...
    #
    $HTTP["url"] =~ "(?:ui/local/dynamic/velop/setup.html)|(?:fwupdate.html)|(?:\.cgi)" {
]])
...
server.max-request-size = 200000             # max file size in kbytes, nodes has a large fw image
]])
...
if smartMode == 1 and masterIpAddress then
    io.stdout:write(([[
# Device is a slave. Need to redirect root request to Master's and /CA to local.

url.rewrite += ( "^/(CA|ca)$" => "/ui/local/dynamic/index.html" )
...
$SERVER["socket"] == ":11161" { # IGNORE-PORT
