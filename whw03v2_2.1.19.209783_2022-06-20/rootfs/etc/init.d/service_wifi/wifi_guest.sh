...
wifi_guest_start ()
{
	...
	if [ "$BRIDGE_MODE" != "0" -a "`syscfg get wifi_bridge::mode`" = "0" ] ; then
	...
	if [ -f /tmp/"$SYSCFG_INDEX"_guest_settings.conf ]; then
		GUEST_ENABLED=`grep -s ^guest_enabled /tmp/"$SYSCFG_INDEX"_guest_settings.conf` 
		GUEST_ENABLED=${GUEST_ENABLED#*: }
		WLX_GUEST_ENABLED=`grep -s ^"$SYSCFG_INDEX"_guest_enabled /tmp/"$SYSCFG_INDEX"_guest_settings.conf`
		WLX_GUEST_ENABLED=${WLX_GUEST_ENABLED#*: }
	else
	...
	if [ "$SYSEVENT_BACKHAUL_INTF" = "ath9" -a "`cat /etc/product`" = "nodes" ] && [ "$SYSCFG_INDEX" = "wl1" ] ; then
		echo "${SERVICE_NAME}, ${SYSCFG_INDEX}_state=$USER_STATE, do not start guest on the backhaul $PHY_IF"
		return 1
	fi
	if [ "$SYSEVENT_BACKHAUL_INTF" = "ath9" -a "`cat /etc/product`" = "rogue" -a "`cat /etc/product`" = "lion" ] && [ "$SYSCFG_INDEX" = "wl1" ] ; then
		echo "${SERVICE_NAME}, ${SYSCFG_INDEX}_state=$USER_STATE, do not start guest on the backhaul $PHY_IF"
		return 1
	fi
...
guest_start()
{
	...
	set_countryie $GUEST_VAP
	iwpriv $GUEST_VAP wds 1
	iwpriv $GUEST_VAP shortgi 1
	iwpriv $GUEST_VAP vlan_tag 1
	iwpriv $GUEST_VAP rrm 1
	if [ "wl0" = "$SYSCFG_INDEX" ]; then
		set_11ngvhtintop $GUEST_VAP
	fi
	return 0
}
guest_stop()
...
configure_guest()
{
	...
	if [ "`cat /etc/product`" = "nodes" -o "`cat /etc/product`" = "nodes-jr" -o "`cat /etc/product`" = "rogue" -o "`cat /etc/product`" = "lion" ] && [ "$SYSCFG_INDEX" = "wl2" ] ; then
		 GUEST_SSID=`syscfg_get wl1_guest_ssid` 
		 syscfg set wl2_guest_ssid $GUEST_SSID
	fi
	...
	if [ "`cat /etc/product`" = "nodes" -o "`cat /etc/product`" = "nodes-jr" -o "`cat /etc/product`" = "rogue" -o "`cat /etc/product`" = "lion" ] ; then
		if [ "$SYSCFG_INDEX" = "wl0" ]; then
			WL_PASSPHRASE=`syscfg_get guest_password`
		else
			WL_PASSPHRASE=`syscfg_get "$SYSCFG_INDEX"_guest_password`
		fi
		if [ ${#WL_PASSPHRASE} = 64 ]; then 
			GUEST_PASSPHRASE="wpa_psk=${WL_PASSPHRASE}"
		else
			GUEST_PASSPHRASE="wpa_passphrase=${WL_PASSPHRASE}"
		fi
		HOSTAPD_GUEST="/tmp/hostapd-$GUEST_VAP.conf"
		generate_hostapd_config $GUEST_VAP "$GUEST_SSID" "$GUEST_PASSPHRASE" "2" "CCMP" "" "" ""> $HOSTAPD_GUEST
	fi
	driver_update_extra_virtual_settings $PHY_IF $GUEST_VAP
	return 0
}
