...
--
-- $Author: jianxiao $
-- $DateTime: 2021/11/23 19:53:10 $
-- $Id: //depot-irv/olympus/nodes_dev_tb/lego_overlay/proprietary/jnap/lualib/platform.lua#5 $
--
...
_M.GET_NUMERIC_TIMEZONE_COMMAND = 'date +%z'
...
--
-- return the numeric timezone in the format [+-]HHMM (example: -0700)
-- Input = CONTEXT
-- Output = string
--
function _M.getNumericTimeZone(sc)
    sc:readlock()

    -- Set the TZ environment variable, so the spawned process environment has the correct TZ
    local file = io.popen(string.format('TZ="%s" ', sc:get_time_zone()) .. _M.GET_NUMERIC_TIMEZONE_COMMAND)
    if file then
        local data = file:read('*a')
        file:close()
        local tz = data:match('[-+]%d%d%d%d')
        if tz then
            return tz
        end
    end
    error(('unable to parse output of "%s"'):format(_M.GET_NUMERIC_TIMEZONE_COMMAND))
end
...
function _M.getWirelessStationList(sc, iface, isGuest)
    ...
    if getSTAList then
        if isMtk then
            ...
            -- param_name = 'stainfo'
            --
            -- 2.4GHz   : MAC                AID BSS PSM WMM MIMOPS  RSSI0  RSSI1  RSSI2  PhMd      BW    MCS   SGI   STBC  Idle   Rate
            --            E4:98:D6:1B:5B:CB  1   0   1   1   0       -50    -43    0      HTMIX     20M   7     0     0     299    65     10        , 18, 44%
            -- 5GHz     : MAC                AID BSS PSM WMM MIMOPS  RSSI0  RSSI1  PhMd      BW    MCS   SGI   STBC  Idle   Rate   TIME
            --            E4:98:D6:1B:5B:CB  1   0   1   1   0       -56    -54    HTMIX     40M   7     0     0     298    270    31     0         , 51, 100%
            local str = getSTAList:read('*line')
            if str and string.match(str, 'parameter stainfo') then
                for line in getSTAList:lines() do
                    -- 2.4GHz output
                    for macAddress, rssi, rate in string.gmatch(line, '(%x%x:%x%x:%x%x:%x%x:%x%x:%x%x)%s+%d+%s+'..(isGuest and 1 or 0)..'%s+%d+%s+%d+%s+%d+%s+([-]*%d+)%s+[-]*%d+%s+[-]*%d+%s+%u+%s+%d+M%s+%d+%s+%d+%s+%d+%s+%d+%s+(%d+).*') do
                    --                                                  MACADDRESS                       AID       BSS                      PSM   WMM   MIMOPS  RSSI0      RSSI1       RSSI2  PhMd  BW     MCS   SGI   STBC  Idle   Rate
                    --                                                  E4:98:D6:1B:5B:CB                1         0                        1     1     0      -50         -43          0     HTMIX 20M    7     0     0     299    65     10    , 18, 44%
                    ...
                    -- 5GHz output
                    for macAddress, rssi, rate in string.gmatch(line, '(%x%x:%x%x:%x%x:%x%x:%x%x:%x%x)%s+%d+%s+'..(isGuest and 1 or 0)..'%s+%d+%s+%d+%s+%d+%s+([-]*%d+)%s+[-]*%d+%s+%u+%s+%d+M%s+%d+%s+%d+%s+%d+%s+%d+%s+(%d+).*') do
                    --                                                  MACADDRESS                       AID       BSS                      PSM   WMM   MIMOPS  RSSI0   RSSI1 PhMd  BW     MCS   SGI   STBC  Idle   Rate   TIME
                    --                                                  E4:98:D6:1B:5B:CB                1         0                        1     1     0      -56     -54    HTMIX 40M    7     0     0     298    270    31    0    ,   51, 100%
                        table.insert(stations, {
                            macAddress = hdk.macaddress(macAddress),
                            mode = nil,
